      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            Combined account ledger: invoices + Customer Service jobs, payments, opening/closing balance
          </p>
        </div>
        {statement && (
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <Card className="border-emerald-200 bg-emerald-50/50">
              <CardContent className="p-3">
                <p className="text-[10px] font-medium text-slate-600">Total Billed (Invoices + Jobs)</p>
                <p className="text-lg font-bold text-emerald-700">Rs.{formatCurrency(statement.summary?.totalBilled || 0)}</p>
                <p className="text-[9px] text-slate-500 mt-0.5">
                  Inv: Rs.{formatCurrency(statement.summary?.totalInvoiced || 0)} · Jobs: Rs.{formatCurrency(statement.summary?.totalJobCharges || 0)}
                </p>
              </CardContent>
            </Card>
            <Card className="border-purple-200 bg-purple-50/50">
              <CardContent className="p-3">
                <p className="text-[10px] font-medium text-slate-600">Total Received (Inv + Service Pmt)</p>
                <p className="text-lg font-bold text-purple-700">Rs.{formatCurrency(statement.summary?.totalReceived || 0)}</p>
                <p className="text-[9px] text-slate-500 mt-0.5">
                  Inv: Rs.{formatCurrency(statement.summary?.totalPaid || 0)} · Svc: Rs.{formatCurrency(statement.summary?.totalServicePaid || 0)}
                </p>
              </CardContent>
            </Card>
...
<body>
  ...
  <div class="summary">
    ...
    <div class="summary-card"><div class="label">Total Billed (Inv + Jobs)</div><div class="value" style="color:#16a34a">Rs.${formatCurrency(s.summary?.totalBilled || 0)}</div></div>
    <div class="summary-card"><div class="label">Total Received (Inv + Svc Pmt)</div><div class="value" style="color:#7c3aed">Rs.${formatCurrency(s.summary?.totalReceived || 0)}</div></div>
  ...
  <p style="font-size:10px;color:#64748b;margin:-5px 0 10px">
    Invoices: Rs.${formatCurrency(s.summary?.totalInvoiced || 0)} ·
    Service Jobs: Rs.${formatCurrency(s.summary?.totalJobCharges || 0)} ·
    Invoice Payments: Rs.${formatCurrency(s.summary?.totalPaid || 0)} ·
    Service Payments: Rs.${formatCurrency(s.summary?.totalServicePaid || 0)}
  </p>
  <table>
