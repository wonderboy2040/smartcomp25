 ...
 * (explicit customer gender if set, else inferred from the customer name).
 */
...
/** v13.7: greeting with WhatsApp bold (*) markers on the honorific/name. */
function boldGreeting(rawName?: string, gender?: string): string {
  const line = respectfulGreeting(rawName, gender)
  // "Respected Sir," → "Respected *Sir*," | "Dear Ramesh," → "Dear *Ramesh*,"
  return line.replace(/^(Respected|Dear) (.+),$/, '$1 *$2*,')
}
...
export function buildWhatsAppMessage(
  ...
  // v13.7 — respectful greeting with WhatsApp bold markers:
  //   "Respected *Sir*," / "Respected *Madam*," / "Dear *Ramesh*,"
  const greeting = boldGreeting(job.customerName, job.customerGender)
