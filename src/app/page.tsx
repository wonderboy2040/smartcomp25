...
// firing it together with the low-priority tail made all 9 Firestore reads
...
function HomeInner() {
  ...
  // Periodic dashboard refresh — pauses when tab is hidden (saves battery + Firestore quota)
  useEffect(() => {
    if (!isConfigured) return
  ...
  useEffect(() => {
    let cancelled = false
    // Fetch /api/config (public) to check if Firebase credentials are set.
    ...
    Promise.all([
      ...
      .then(([configData, authData]) => {
        ...
        // If Firebase is NOT configured → show SetupWizard (lets user paste creds)
